...
# -*- coding: utf-8 -*-
"""
MATWM Training Base Module
Common training function for all experiments
"""
...
# Export config classes for convenience
...
def make_env(max_cycles=25, seed=None):
    """Create PettingZoo simple_tag environment"""
...
def train_matwm_with_curiosity(config, curiosity_config, save_dir='results', resume_from=None):
    ...
    Args:
        ...
        save_dir: Save directory (will be created with timestamp subdirectory)
        resume_from: Checkpoint path to resume from
    
    Returns:
        tuple: (agents, episode_rewards, training_metrics, episode_curiosity, curiosity_metrics)
    """
    print('=' * 70)
    ...
    print(f'LLM logs: {curiosity_config.log_dir}\n')
    ...
    while global_step < config.total_steps:
        ...
        for step in range(config.max_cycles):
            ...
            if global_step % config.save_interval == 0 and global_step >= config.warmup_steps:
                ckpt_dir = os.path.join(save_dir, f'checkpoint_{global_step}')
