...
# -*- coding: utf-8 -*-
"""
MATWM Training Script with use_gamma_progress=False
Converted from Jupyter Notebook and cleaned for GCP execution
"""
...
import numpy as np
import matplotlib
matplotlib.use('Agg')  # For non-GUI environment (GCP)
...
print('=' * 70)
print('MATWM + Curiosity-Driven Training (γ-Progress=FALSE)')
print('=' * 70)
...
# Configuration - Paper reproduction settings
config = MATWMConfig(
    total_steps=50000,      # Paper reproduction
    warmup_steps=1000,      # Paper reproduction (increased from 1000 for stability)
    log_interval=100,
    save_interval=1000,
    use_gamma_progress=False,  # ★ FALSE for this script
)

print(f'\nuse_gamma_progress: {config.use_gamma_progress}')
print(f'Total steps: {config.total_steps}')
print(f'Warmup steps: {config.warmup_steps}')
print(f'Social curiosity enabled')
print('=' * 70)
...
# Training Function
# ============================================================
def train_matwm_with_curiosity(config, curiosity_config, save_dir='results_gamma_false', resume_from=None):
    """
    ...
    # Shared World Model (γ-Progress disabled)
    ...
    while global_step < config.total_steps:
        ...
        for step in range(config.max_cycles):
            ...
            # Curiosity reward calculation and experience storage
            for name, agent in agents.items():
                other_acts = {k: v for k, v in actions.items() if k != name}
                env_r = rewards[name]
                
                # Computational curiosity (activated after WM is trained)
                intrinsic_r = 0.0
                ...
                # Store in replay buffer (env + curiosity reward)
            ...
            if global_step >= config.warmup_steps:
                ...
                # γ-Progress: Update World Model EMA
        ...
        # Episode end: LLM semantic curiosity evaluation
        for name in agent_names:
...
if __name__ == '__main__':
    ...
    agents, episode_rewards, training_metrics, episode_curiosity, curiosity_metrics = \
        train_matwm_with_curiosity(config, curiosity_config, save_dir='results_gamma_false')
    
    print(f'\n[OK] All results saved to: results_gamma_false/')
